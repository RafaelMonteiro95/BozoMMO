			...
			*/
			try{
				do {
					...
					// DEBUG
					System.out.println("Client '" + this.id + "' msg: " + msg);
					...
					if(args.length == 1) { //F e R<n>
						if (args[0].equals("F")) { //F
							...
							// DEBUG
							System.out.println("SENDING @" + this.id + ": " + response);
							...
							response = Arrays.toString(rd.rolar());
							serverOut.println(response);
							// DEBUG
							System.out.println("SENDING @" + this.id + ": " + response);
						}
						...
						response = Arrays.toString(rd.rolar(quais));
						serverOut.println(response);
						// DEBUG
						System.out.println("SENDING @" + this.id + ": " + response);
					}
		...
		public void run(){
			...
			do{
				...
				if(input.nextLine().equals("exit")){
					s.setFlag();
					break;
				} else if(input.nextLine().equals("clear"))
					clear();
			} while(true);
